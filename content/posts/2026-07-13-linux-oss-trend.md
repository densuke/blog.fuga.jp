...
正直、ここだけ見るとかなりニッチな話に見えます。でもこの廃止、証明書の世代交代のすぐ後に読むと腹落ちしやすいので、あえてこの並びで置きました。ユーザー空間からカーネル暗号API（AES、SHAなど）へroot権限なしに直接アクセスできる`AF_ALG`ソケットが、Linux 7.2で正式に非推奨化されました。GoogleのEric Biggersが「巨大な攻撃面」と断じ、2026年5月にゼロコピーパス削除、6月に廃止ドキュメントと`DEPRECATED`マーク付けという二段階のパッチが入っています。
